...
class Modbus {
private:
  SoftwareSerial *port; //!< Pointer to Serial class object
  uint8_t u8id; //!< 0=master, 1..247=slave number
  uint8_t _serialRX;
  uint8_t _serialTX;
  uint8_t _serialTxControl;
  uint8_t u8state;
  uint8_t u8lastError;
  ...
  void init(uint8_t u8id, uint8_t serialRX, uint8_t serialTX, uint8_t serialTxControl);
  void sendTxBuffer(); 
  int8_t getRxBuffer(); 
...
public:
  Modbus(uint8_t u8id, uint8_t serialRX, uint8_t serialTX, uint8_t serialTxControl);
 ...
 */
Modbus::Modbus(uint8_t u8id, uint8_t serialRX, uint8_t serialTX, uint8_t serialTxControl) {
  init(u8id, serialRX, serialTX, serialTxControl);
}
...
void Modbus::begin(long u32speed) {
  ...
  port = new SoftwareSerial(this->_serialRX, this->_serialTX);

  // port->begin(u32speed, u8config);
  port->begin(u32speed);
  pinMode(this->_serialTxControl, OUTPUT);
  digitalWrite(this->_serialTxControl, RS485Transmit);
  port->println("Modbus Begin");
  delay(10);
  digitalWrite(this->_serialTxControl, RS485Receive);
...
void Modbus::init(uint8_t u8id, uint8_t serialRX, uint8_t serialTX, uint8_t serialTxControl) {
  this->u8id = u8id;
  this->_serialRX = serialRX;
  this->_serialTX = serialTX;
  this->_serialTxControl = serialTxControl;
